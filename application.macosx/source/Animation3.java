...
public class Animation3 extends PApplet {

float radius = 50.0f;
float ball_speed = 0.0f;
float sun_speed = 0.0f;
float racket_pos = -400.0f;
float last_sec = 0.0f;
int keyR_count = 0;
int[] rain_speed;
int[] rain_pos;
...
public void setup()
{
  ...
  for(int i = 0; i < 100; i++){
    rain_speed[i] = PApplet.parseInt(random(10,30));
    rain_pos[i] = PApplet.parseInt(random(0,100));
  }// init
}
...
public void draw()
{
  ...
  if(key4 == true)
    rotateY(radians(frameCount)*0.5f);  //free view
...
public void moveView()
{
  if(keyEnter){
    if(rotateView < 150){
      rotateX(radians(rotateView)*0.5f);
      rotateView++;
    }
  }// pressed enter key
...
public void drawSky()
{
  ...
  float skyT = (sun_speed/1000.0f)%1;
...
public void drawRain()
{
  ...
  for(int i = 0; i < 100; i++){
    for(int pos = -500; pos <= 500; pos += 100)
      line(rain_pos[i], i*10, pos, rain_pos[i] + rain_speed[i]*1.5f, i*10, pos);
...
public void keyPressed()
{
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Animation3" };
    if (passedArgs != null) {
